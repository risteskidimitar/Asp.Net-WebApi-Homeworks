...
namespace SEDC.WebApi.NoteApp.Services.Helpers
{
    public static class DiModule
    {
            ...
            string connectionString)
        {
            ...
            //services.AddTransient<IRepository<Note>>(x => new AdoNoteRepository(connectionString));

            // Dapper reops
            services.AddTransient<IRepository<User>>(
                x => new DapperUserRepository(connectionString));

            services.AddTransient<IRepository<Note>>(x => new DapperNoteRepository(connectionString));
